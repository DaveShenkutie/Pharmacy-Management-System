...
$getPresid=$con->query("SELECT 1+MAX(prescription_id) FROM prescription");
$presId=$getPresid->fetch();
		if($presId[0]=='')
		{
			$presIdd=999;
		 }
	...
	$sqlP=$con->query("INSERT INTO prescription(prescription_id,customer_id,customer_name,age,sex,postal_address,invoice_id,phone)
				VALUES('{$presIdd}','{$c_id}','{$cname}','{$age}','{$sex}','{$postal}','{$invoiceNo}','{$phone}')  ");
		$sqlI=$con->query("INSERT INTO invoice(invoice_id, customer_name,served_by,status)
				VALUES('{$invoiceNo}','{$cname}','{$user}','Pending') ");
						
$getDetails=$con->query("SELECT * FROM tempprescri WHERE customer_id='{$c_id}'");
while($item1=$getDetails->fetch())
			{	
				$getDetails1=$con->query("SELECT stock_id, cost FROM stock WHERE drug_name='{$item1['drug_name']}'");	
			
				$details=$getDetails1->fetch();
				$sqlId=$con->query("INSERT INTO invoice_details(invoice,drug,cost,quantity)
	...
	fclose($file);
		$getDetails=$con->query("SELECT * FROM tempprescri WHERE customer_id='{$c_id}'");
while($item12=$getDetails->fetch())
			{	
			$getDetails12=$con->query("SELECT stock_id, cost FROM stock WHERE drug_name='{$item12['drug_name']}'");	
			
				$details2=$getDetails12->fecth();
			$sqlIp=$con->query("INSERT INTO prescription_details(pres_id,drug_name,strength,dose,quantity)
				VALUES('{$presIdd}','{$details2['stock_id']}','{$item12['strength']}','{$item12['dose']}','{$item12['quantity']}') ");	
					
			}
		$sqlD=$con->query("DELETE FROM tempprescri WHERE customer_id='{$c_id}' ");
