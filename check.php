...
if(isset($_POST['drug_name']))
{
	...
	$sql=$con->query("INSERT INTO tempPrescri(customer_id,customer_name,age,sex,postal_address,phone,drug_name,strength,dose,quantity)
	...
	$get_cost=$con->query("SELECT cost FROM stock WHERE drug_name='{$drug}'");
	$cost=$get_cost->fetch();
	$tot=$quantity*$cost[0];
	...
	echo "<table width=\"100%\" border=1>";
        ...
        // loop through results of database query, displaying them in the table
		 $result =$con->query("SELECT * FROM tempPrescri")or die(mysql_error());
...
$getDetails=$con->query("SELECT invoice,drug,cost,quantity FROM invoice_details WHERE invoice='{$invoice}'");
$getQuantity=$con->query("SELECT quantity FROM invoice_details WHERE id='{$invoice}'");
$file=fopen("receipts/docs/".$_SESSION['invoiceNo'].".txt", "w");	
	echo "<table width=\"100%\" border=1>";
		...
		<th>Unit cost</th>
		<th>Quantity </th>
		<th>Total Cost(Birr.)</th></tr>";
while($item5=$getDetails->fetch())
			{
			$getDrug=$con->query("SELECT drug_name FROM stock WHERE stock_id='{$item5['drug']}'");
						
			$drug=$getDrug->fetch();
			$qtty=$getQuantity->fetch();
