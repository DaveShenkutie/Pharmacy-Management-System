...
<hr/>	
    <div class="tabbed_area">  
       ...
       $result = $con->query("SELECT * FROM prescription")or die(mysql_error());
...
?> 
			...
			$(document).ready(function()
	{
		...
		<?php
		$invNum= $con->query ("SELECT 1+MAX(invoice_id) FROM invoice");
		...
		<form name="myform" onsubmit="return validateForm(this);" action="invoice.php" method="post" >
			<table width="200" height="106" border="0" >	
				...
				echo"<select  class=\"input-small\" name=\"drug_name\" style=\"width:170px\" id=\"drug_name\">";
						 $getpayType=$con->query("SELECT drug_name FROM stock");
						 echo"<option>Select Drug</option>";
		 while($pType=$getpayType->fetch())
			{
				echo"<option>".$pType['drug_name']."</option>";
			}
		
		echo"</select>";?>  </td></tr>
<tr><td align="left"><input name="strength" type="text" style="width:170px"  id="strength"placeholder="Strength (grams)" /></td></tr>
				<tr><td align="left"><input name="dose" type="text" style="width:170px" id="dose" placeholder="Dose(HMT)" /></td></tr>
