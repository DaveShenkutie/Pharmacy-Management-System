...
if (!preg_match("/^[a-zA-Z ]*$/",$fname))//preg_match(pattern, input, matches, flags, offset)
  {
...
$user=$_POST['username'];
$pas=$_POST['password'];
$sql1=$con->query("SELECT * FROM cashier WHERE username='$user'")or die(mysql_error());
 $result=$sql1->fetch();
 if($result){
$message="<font color=blue>sorry the username entered already exists</font>";
 }else{
$sql=$con->query("INSERT INTO cashier(first_name,last_name,staff_id,postal_address,phone,email,username,password,date)
...
<hr/>	
    <div class="tabbed_area">  
		...
		*/
        ...
        $result = $con->query("SELECT * FROM cashier") 
                or die(mysql_error());
