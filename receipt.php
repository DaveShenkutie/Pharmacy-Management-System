...
if(isset($_POST['save']))
{
	$result = $con->query("SELECT COUNT(*) FROM receipts");
	$last=$result->fetchColumn();
...
$getCid=$con->query("SELECT customer_id FROM prescription WHERE invoice_id='{$invNo}' ");
$getName=$con->query("SELECT customer_name FROM invoice WHERE invoice_id='{$invNo}' ");
$details1=$getName->fetch(); 
$details=$getCid->fetch();
$cid=$details['customer_id'];
	
	$sqlP=$con->query("INSERT INTO receipts(reciptNo,customer_id,total,payType,serialno,served_by)
				VALUES('{$recNo}','{$cid}','{$amount}','{$pType}','{$serial}','{$_SESSION['username']}') ");
...
Eshetu Chole second Floor,<br> P.O. Box 1000 Taytu Bitul, Ethiopia <br> Tel: +251 946 69 0749  <br> E-mail: Dawitshenkutie@yahoo.com 
<br>-----------------------------------------
EOD;
$header = array('Drug','Price.', 'Qtty' ,'Cost(Birr)');
$ddt=<<<EOD
$time   
EOD;
$html = <<<EOD
