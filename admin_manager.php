...
if(isset($_POST['submit'])){
	...
	$user=$_POST['username'];
	$pas=$_POST['password'];
	$sql1=$con->query("SELECT * FROM manager WHERE username='$user'")or die(mysql_error());
	$result=$sql1->fetch();
 if($result){
	$message="<font color=blue>sorry the username entered already exists</font>";
 }else{
	$sql=$con->query("INSERT INTO manager(first_name,last_name,staff_id,postal_address,phone,email,username,password,date)
	VALUES('$fname','$lname','$sid','$postal','$phone','$email','$user','$pas',NOW())");
	if($sql) {
		header("location:http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF'])."/admin_manager.php");
	}
	else
	{
